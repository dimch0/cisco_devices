...
FILE_CONTAINER = "/home/dzhrt/PycharmProjects/untitled/pyt/cisco_devices/PGI"
EXTENSION = ".txt"
...
def cisco_vs_juniper_excel(MASTER_FILE):
    """
...
def cisco_vs_juniper_files():
    ...
    :return: two lists (for cisco and juniper) with the full paths
    to each of the files
    """
    indicating_string = "Hardware inventory:"
    cisco_file_list = []
    juniper_file_list = []
    for file in get_all_files(FILE_CONTAINER):
...
def cisco_reader(dir_to_file):
    ...
    :return: a list of tuples with PIDs and SNs:
    [(pid, sn), (pid, sn), (pid, sn)...]
    """
        ...
        for line in f:
            ...
            for i, item in enumerate(line):
                try:
                    ...
                    print "Error: Could not print cisco PID, SN {0}",
                    e,
                    "in file: ", file_name
        ...
        for line in f:
            ...
            for i, item in enumerate(line):
                try:
                    ...
                    print "Error: Could not print juniper PID, SN",
                    e,
                    "in file: ",
                    file_name
            if pid or sn:
...
def create_dict(file_list, reader):
    ...
    :return:  a dict with the name of the file as a key and
    list of paris (pid, sn) for value:
...
def write_missing_entries(missing_names_list, all_dict, sheet, header):
    """
    This function writes the missing entries from the master in a excel sheet
...
def main():
    ...
    cisco_names = cisco_vs_juniper_excel(MASTER_FILE)[0]
    juniper_names = cisco_vs_juniper_excel(MASTER_FILE)[1]
    other_devices = cisco_vs_juniper_excel(MASTER_FILE)[2]

    # CREATING SETS FOR CISCO AND JUNIPER TO FIND DIFF
    set_cisco_files = set(os.path.basename(item).replace(EXTENSION, '') for
                          item in cisco_file_list)
    set_juniper_files = set(os.path.basename(item).replace(EXTENSION, '') for
                            item in juniper_file_list)
    ...
    write_missing_entries(missing_names_for_cisco,
                          all_dict, worksheet_2,
                          format_header)

    # WRITING MISSING JUNIPER ENTRIES
    write_missing_entries(missing_names_for_juniper,
                          all_dict, worksheet_3,
                          format_header)
